...
function managedAgentsSection(fallbackInvocation?: string): string {
  ...
  lines.push(
    ...
    '- For multi-step work, use `code-kg work` instead of markdown TODOs: `work ready`, `work start <id> --worktree`, `work close <id>`.',
    ...
    '- Run `unslop` over commit/PR prose you write for humans before posting.',
    SECTION_END,
    '',
  );
  return lines.join('\n');
